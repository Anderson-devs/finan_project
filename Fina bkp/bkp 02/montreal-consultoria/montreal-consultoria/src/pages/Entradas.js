...
import { Row, Col, Form, Button, Container, Modal, Toast, Table, ListGroup, Card } from 'react-bootstrap';
...
function Entradas() {
  ...
  const [newClient, setNewClient] = useState({ name: '', phone: '', whatsapp: '', email: '' });
  ...
  const handleClientBlur = () => {
    ClientController.searchClients(cliente).then(results => {
      if (results.length === 0 && cliente) {
        setNewClient({ name: cliente, phone: '', whatsapp: '', email: '' });
  ...
  const handleCreateClient = async () => {
    ...
    setNewClient({ name: '', phone: '', whatsapp: '', email: '' });
  };
          ...
          <Card className="entradas-card mb-4">
            <Form onSubmit={handleSubmit} className="entradas-form">
              <Form.Group className="mb-3 position-relative">
                ...
                />
                {showClientSuggestions && clienteOptions.length > 0 && (
